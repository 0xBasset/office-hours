...
contract Calendar {
    ...
    function isCalendarDay(uint256 time, uint256 secondsFromBase) public pure returns (bool isDay) {
        uint256 timezoneBase = base - _offsetForTimezone(25);
    ...
    function calendarOffset(uint256 profId) public pure returns (uint256 off_) {
        // Santa
        if (profId == 20) off_ = 17452800;
        // Hedgehog
        if (profId == 21) off_ = 0;
        // Uncle Sam
        if (profId == 22) off_ = 2419200;
        // Jack-o-Lantern
        if (profId == 23) off_ = 12700800;
    }
    ...
    function getCalendar(uint256 profId) public pure returns(uint256 weekdays, uint256 start, uint256 duration, uint256 calendar) {
        ...
        // Athlete
        if (profId == 12) ( weekdays, start, duration, calendar) = ( 68, 16 hours, 6 hours, 0);
        // Actress
        if (profId == 13) ( weekdays, start, duration, calendar) = (106, 10 hours, 7 hours, 0);
        // Fashion Designer
        if (profId == 14) ( weekdays, start, duration, calendar) = (21, 10 hours, 7 hours, 0);
        // Doctor
        if (profId == 15) ( weekdays, start, duration, calendar) = ( 42, 7 hours, 15 hours, 0);
        // Judge
        if (profId == 16) ( weekdays, start, duration, calendar) = (30, 11 hours, 5 hours, 0);
        // Rugpuller
        if (profId == 17) ( weekdays, start, duration, calendar) = (127, 1 hours, 64 hours, 0);
        // Ninja
        if (profId == 18) ( weekdays, start, duration, calendar) = ( 68, 0 hours, 6 hours, 0);
        // Congo Warlord
        if (profId == 19) ( weekdays, start, duration, calendar) = (0, 0 , 0 , 0);
        // Mech Santa
        if (profId == 20) ( weekdays, start, duration, calendar) = (0, 0 , 0 , 17452800);
        // Hedgehog
        if (profId == 21) ( weekdays, start, duration, calendar) = (0, 0 , 0 , 0);
        // Uncle Sam
        if (profId == 22) ( weekdays, start, duration, calendar) = (0, 0 , 0 , 2419200);
        // Jack-o-Lantern
        if (profId == 23) ( weekdays, start, duration, calendar) = (0, 0 , 0 , 12700800);
    }
    ...
    function rates(uint256 profId) external pure returns(uint8 start, uint8 end) {
        ...
        // Athlete
        if (profId == 12) ( start, end ) = (4, 15);
        // Actress
        if (profId == 13) ( start, end ) = (3, 11);
        // Fashion Designer
        if (profId == 14) ( start, end ) = (3, 9);
        // Doctor
        if (profId == 15) ( start, end ) = (9, 12);
        // Judge
        if (profId == 16) ( start, end ) = (8, 12);
        // Rugpuller
        if (profId == 17) ( start, end ) = (9, 17);
        // Ninja
        if (profId == 18) ( start, end ) = (10, 14);
        // Congo Warlord
        if (profId == 19) ( start, end ) = (12, 25);
        // Mech Santa
        if (profId == 20) ( start, end ) = (15, 25);
        // Hedgehog
        if (profId == 21) ( start, end ) = (1, 3);
        // Uncle Sam
        if (profId == 22) ( start, end ) = (10, 13);
        // Jack-o-Lantern
        if (profId == 23) ( start, end ) = (11, 14);
    }
